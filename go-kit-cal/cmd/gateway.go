...
import (
	...
	"github.com/baxiang/go-note/go-kit-trace/routers"
...
func main() {
	...
	var (
		...
		consulHost = flag.String("consul.host", hostIP, "consul server ip address")
		consulPort = flag.String("consul.port", "8500", "consul server port")
		zipkinURL  = flag.String("zipkin.url", fmt.Sprintf("http://%s:9411/api/v2/spans",hostIP), "Zipkin server url")
	)
	flag.Parse()
	...
	hystrixRouter := routers.Routes(consulClient, zipkinTracer, "Circuit Breaker:Service unavailable", logger)
	...
	go func() {
		errChan <- http.ListenAndServe(":8089", hystrixStreamHandler)
	}()
	...
	go func() {
		...
		errChan <- http.ListenAndServe(":8088", handler)
	}()
